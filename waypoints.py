...
def get_waypoints(route, v, d=15, dt=0.2):
    random.seed(time.time())
    vRand = 1000/(1000/v-(2*random.random()-1)*d)
    fixedLoc = fixLockT(route, vRand, dt)
    nList = (5, 6, 7, 8, 9)
    n = nList[random.randint(0, len(nList)-1)]
    fixedLoc = randLoc(fixedLoc, n=n)
    points = []
    for i in fixedLoc:
        points.append(bd09Towgs84(i))
    return points

def waypoints(v):
    route = get_route()
    waypoints = get_waypoints(route, v)
    return waypoints

# if __name__ == "__main__":
#     loc = get_route()
#     waypoints = get_waypoints(loc, 3)
#     print(waypoints)